...
const interviews = require('../schemas/interviews.schema')
...
exports.analytics = async function(req,res){
  ...
  var tag = ["stat","barChart","pieChart"]
  var finalData = {}
  var url = [`http://localhost:5000/stat?month=${m}`,`http://localhost:5000/barChart?month=${m}`,`http://localhost:5000/pieChart?month=${m}`]
   for(let i=0; i < url.length ;i++){
 
   
    var data  = await axios.get(url[i])
    finalData[tag[i]] = data.data
   }
...
exports.stat = async function(req,res){
    var m = parseInt(req.query.month);
    var data = await interviews.aggregate([{$project:{sold:1,price:1,dateOfSale:{$month:'$dateOfSale'}}},{$match:{dateOfSale:m}}
]);
var totalAmount=0,itemSold=0,itemNotSold=0;
for(let i = 0;i < data.length; i++){
   totalAmount = totalAmount+parseFloat(data[i].price)
   if(data[i].sold == "true"){
    itemSold++;
   }else{
    itemNotSold++;
    ...
    res.send({totalAmount:totalAmount,itemSold:itemSold,itemNotSold:itemNotSold})

}



exports.pieChart = async function(req,res){
    var m = parseInt(req.query.month);
    var data = await interviews.aggregate([{$project:{category:1,price:1,dateOfSale:{$month:'$dateOfSale'}}},{$match:{dateOfSale:m}}
]);
 var category = {} 
 for(let i = 0; i < data.length; i++){
   if(category[data[i].category]){
    category[data[i].category] = category[data[i].category] + 1; 
   }else{
...
exports.barChart = async function(req,res){
    var m = parseInt(req.query.month);
    var data = await interviews.aggregate([{$project:{sold:1,price:1,dateOfSale:{$month:'$dateOfSale'}}},{$match:{dateOfSale:m}}
]);
var chartData = {"0-100":0,"101-200":0,"201-300":0,"301-400":0,"401-500":0,"501-600":0,"601-700":0,"701-800":0,"801-900":0,"901-above":0};
for(let i = 0;i < data.length; i++){
   if(0 < parseFloat(data[i].price) <= 100){
       chartData["0-100"] = chartData["0-100"] + 1
   }
   else if(100 < parseFloat(data[i].price) <= 200){
    chartData["101-200"] = chartData["101-200"] + 1
    }
   else if(200 < parseFloat(data[i].price) <= 300){
        chartData["201-300"] = chartData["201-300"] + 1
    }
    else if(300 < parseFloat(data[i].price) <= 400){
        chartData["301-400"] = chartData["301-400"] + 1
    }
    else if(400 < parseFloat(data[i].price) <= 500){
        chartData["401-500"] = chartData["401-500"] + 1
    }
    else if(500 < parseFloat(data[i].price) <= 600){
        chartData["501-600"] = chartData["501-600"] + 1
    }
    else if(600 < parseFloat(data[i].price) <= 700){
        chartData["601-700"] = chartData["601-700"] + 1
    }
    else if(700 < parseFloat(data[i].price) <= 800){
        chartData["701-800"] = chartData["701-800"] + 1
    }
    else if(800 < parseFloat(data[i].price) <= 900){
        chartData["801-900"] = chartData["801-900"] + 1
    }
    else {
        chartData["901-above"] = chartData["900-above"] + 1
    }
